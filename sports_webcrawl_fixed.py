...
if favorite_team in espn_sports[favorite_sport]:
    ...
    if roster:
        ...
        print("-" * 95)
        print(f"{'Name':<25} {'Jersey':<7} {'Pos':<4} {'Age':<4} {'Height':<7} {'Weight':<8} {'College':<20}")
        print("-" * 95)
        for player in roster:
            jersey = f"#{player['number']}" if player['number'] != 'N/A' else 'N/A'
            print(f"{player['name']:<25} {jersey:<7} {player['position']:<4} {player['age']:<4} {player['height']:<7} {player['weight']:<8} {player['college']:<20}")
